...
bool ClipPlaneShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	bool result = InitializeShader(device, hwnd, L"./Shaders/Clipplane_Vertex.hlsl", L"./Shaders/Clipplane_Pixel.hlsl");
	return result;
}
...
bool ClipPlaneShaderClass::Render(ID3D11DeviceContext* deviceContex, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* textureResource, D3DXVECTOR4 clipPlane)
{
	// Set the shader parameters that it will use for rendering
	bool result = SetShaderParameters(deviceContex, worldMatrix, viewMatrix, projectionMatrix, textureResource, clipPlane);
	if (!result)
	{
		return false;
	}
...
bool ClipPlaneShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFileName, WCHAR* psFileName)
{
	...
	result = D3DX11CompileFromFile(vsFileName, NULL, NULL, "ClipplaneVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFile(psFileName, NULL, NULL, "ClipplanePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
...
bool ClipPlaneShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* textureResource, D3DXVECTOR4 clipPlane)
{
	HRESULT result;
	...
	// Copy the clip buffer into the constant buffer
