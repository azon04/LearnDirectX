...
bool DebugWindowClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// Give the subresource structure a pointer to vertex data
	...
	// Release the arrays no that the vertex and index buffers have been created and loaded
	delete[] vertices;
	delete[] indices;
...
bool DebugWindowClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	// if position we are rendering this bitmap to has not changed then don't update the vertex buffer since it
	// currently has the correct parameter
	...
	// Calculate the left side of the bitmap
	left = (float)((m_screenWidth / 2) * -1) + (float)positionX;

	// Calculate the right side of the bitmap
	right = left + (float)m_bitmapWidth;

	// Calculate the top side of the bitmap
	top = (float)(m_screenHeight / 2) - (float)positionY;

	// Calculate ht bottom side of the bitmap
...
void DebugWindowClass::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	// set the type of primitive that should be rendererd from this vertex buffer, in this case triangle
