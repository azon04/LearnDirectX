...
bool TransparentShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	bool result = InitializeShader(device, hwnd, L"./Shaders/Transparent_Vertex.hlsl", L"./Shaders/Transparent_Pixel.hlsl");
	return result;
}
...
bool TransparentShaderClass::Render(ID3D11DeviceContext* deviceContex, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* textureResource, float blend)
{
	// Set the shader parameters that it will use for rendering
	bool result = SetShaderParameters(deviceContex, worldMatrix, viewMatrix, projectionMatrix, textureResource, blend);
	if (!result)
	{
		return false;
	}
...
bool TransparentShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFileName, WCHAR* psFileName)
{
	...
	result = D3DX11CompileFromFile(vsFileName, NULL, NULL, "TransparentVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFile(psFileName, NULL, NULL, "TransparentPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	// Setup the description of the dynamic transparent constant buffer that is in the pixel shader.
	...
	// Create the constant buffer pointer so that we can access the pixel shader constant buffer from within this class
...
bool TransparentShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* textureResource, float blend)
{
	HRESULT result;
	...
	// Now set the blend constant buffer in the pixel shader with the updated values
