...
void FontClass::BuildVertexArray(void* vertices, char* text, float drawX, float drawY)
{
	...
	for (i = 0; i < numLetters; i++)
	{
		...
		{
			right = drawX + m_Font[letter].size;
			bottom = drawY - 16;
			...
			vertexPtr[index].position = D3DXVECTOR3(right, bottom, 0.0f); // Bottom right
			vertexPtr[index].texture = D3DXVECTOR2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = D3DXVECTOR3(drawX, bottom, 0.0f); // Bottom left
			...
			vertexPtr[index].position = D3DXVECTOR3(right, drawY, 0.0f); // Top right
