...
bool TranslateShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	bool result = InitializeShader(device, hwnd, L"./Shaders/Translate_Vertex.hlsl", L"./Shaders/Translate_Pixel.hlsl");
	return result;
}
...
bool TranslateShaderClass::Render(ID3D11DeviceContext* deviceContex, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* textureResource, float translateOffset)
{
	// Set the shader parameters that it will use for rendering
	bool result = SetShaderParameters(deviceContex, worldMatrix, viewMatrix, projectionMatrix, textureResource, translateOffset);
	if (!result)
	{
		return false;
	}
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFileName, WCHAR* psFileName)
{
	...
	result = D3DX11CompileFromFile(vsFileName, NULL, NULL, "TranslateVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFile(psFileName, NULL, NULL, "TranslatePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	// Setup the description of the dynamic translate constant buffer that is in the pixel shader
	translateBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	translateBufferDesc.ByteWidth = sizeof(MatrixBufferType);
...
bool TranslateShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* textureResource, float translateOffset)
{
	HRESULT result;
	...
	// Copy the translate offset into constant buffer
	dataPtr2->translation = translateOffset;
